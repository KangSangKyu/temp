...
from network import LSTM_FC, RNN_FC, TWO_STACKED_LSTM_FC, THREE_STACKED_LSTM_FC, FOUR_STACKED_LSTM_FC
...
def get_model(config):
    if config.model_fn == 'RNN_FC': 
      model = RNN_FC(num_classes=config.num_classes, 
                     max_features=config.max_features,
                     max_len=config.max_len,
                     units=config.units)
    elif config.model_fn == 'LSTM_FC':
      model = LSTM_FC(num_classes=config.num_classes, 
                      max_features=config.max_features,
                      max_len=config.max_len,
                      units=config.units)
    elif config.model_fn == 'TWO_STACKED_LSTM_FC':
      model = TWO_STACKED_LSTM_FC(num_classes=config.num_classes, 
                      max_features=config.max_features,
                      max_len=config.max_len,
                      units=config.units)
    elif config.model_fn == 'THREE_STACKED_LSTM_FC':
      model = THREE_STACKED_LSTM_FC(num_classes=config.num_classes, 
                      max_features=config.max_features,
                      max_len=config.max_len,
                      units=config.units)
    elif config.model_fn == 'FOUR_STACKED_LSTM_FC':
      model = FOUR_STACKED_LSTM_FC(num_classes=config.num_classes, 
                      max_features=config.max_features,
                      max_len=config.max_len,
                      units=config.units)                                                                  
    else:
...
def main(config):
    ...
    plt.plot(test_acc, '-o')
    plt.title(str(config.model_fn) + '_' + str(config.units) + '_Accuracy')
    plt.legend(['train_acc ' + str(np.max(train_acc)), 'test_acc ' + str(np.max(test_acc))], loc='upper left')
    plt.xlabel('epoch')
    plt.ylabel('Accuracy')
    plt.savefig(str(config.model_fn) + '_' + str(config.units) + '_acc.png')
