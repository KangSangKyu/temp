...
def main(config):
    ...
    for epoch in range(50):
        ...
        if epoch % 1 == 0:
            ...
            print('test_loss:', loss.numpy(), 'test_acc:', metric.result().numpy())
    ...
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.savefig(str(config.model_fn) + '_' + str(config.dropout_ratio) + '_loss.png')
    ...
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
    plt.savefig(str(config.model_fn) + '_' + str(config.dropout_ratio) + '_acc.png')
